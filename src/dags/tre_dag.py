...
def unzip():
    filepath_zip = '/opt/airflow/dags/tre.zip'
    folder_zip = '/opt/airflow/dags'
...
def extract(**kwargs):
    df = pd.read_csv('/opt/airflow/dags/tre.csv', encoding='ISO-8859-1', sep=';')
...
def load(**kwargs):
    ...
    df_unico.to_csv('/opt/airflow/dags/tre_qualificado.csv', index=False, mode='w', sep = ';')
